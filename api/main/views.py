...
from the_fashion_fit.settings import EMAIL_HOST_USER
...
class HomeView(APIView):
    ...
    def get(self, *args, **kwargs):
        popular_items = Item.objects.filter(is_active=True).order_by('-id')[:12]
        recommend_items = Item.objects.filter(is_active=True).order_by('-id')[:8]
        posts = Post.objects.filter(status=1).order_by('-created_on')

        popular_items_serializer = ItemSerializer(popular_items, many=True, context={'user_id': self.request.user.id}).data
        recommend_items_serializer = ItemSerializer(recommend_items, many=True, context={'user_id': self.request.user.id}).data
        posts_serializer = PostSerializer(posts, many=True).data
        context = {
            'all_popular_items':  popular_items_serializer,
            'all_recommended_items':  recommend_items_serializer,
...
class ShopDetailView(APIView):
    ...
    def get(self, *args, **kwargs):
        ...
        # sizes = Size.objects.all().distinct('size_code')
        ...
        sizes_serializer = SizeSerializer(sizes, many=True).data
...
def add_items_to_cart(request, slug):
    ...
    if int(quantity) > 0:
        if int(quantity) <= item.stock_count:
            item.stock_count=item.stock_count- int(quantity)
            item.save()
            order_item, created = OrderItem.objects.get_or_create(
                item=item,
            ...
            if order_qs.exists():
                order = order_qs[0]
                # check if the order item is in the order
                if order.items.filter(item__slug=item.slug, size=Size.objects.get(id=size), color=Color.objects.get(id=color)).exists():
                    ...
                    order_item.size = Size.objects.get(id=size)
